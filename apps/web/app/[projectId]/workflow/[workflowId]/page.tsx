...
const WorkflowXMLSheet = observer(() => {
    const {
        workflow: { modeler },
    } = useMobxStore();

    const [xml, setXml] = useState("");
    const [isSheetOpen, setIsSheetOpen] = useState(false); // New state to track the open state of the Sheet

    useEffect(() => {
        if (!modeler || !isSheetOpen) return; // Only call modeler.saveXML when the Sheet is open
        console.log("[XML] modeler", modeler);
        modeler.saveXML({ format: true }).then(({ xml }) => {
            setXml(xml);
        });
    }, [isSheetOpen]);
    ...
    return (
        <Sheet onOpenChange={(open) => setIsSheetOpen(open)}>
            <SheetTrigger asChild>
                <Button className="flex gap-2.5">XML</Button>
            </SheetTrigger>
            <SheetContent className="w-[600px] sm:w-[740px]">
                <div className="h-full w-full">
                    <ScrollArea className="h-[calc(100%)]">
                        <p className="p-4 whitespace-pre-wrap w-full overflow-auto">
                            {xml}
                        </p>
                    </ScrollArea>
                </div>
